...
The interviewee then noted that this solution is inefficient and repeats work. Iterating through the grid takes \\(O(n^2)\\) time and finding the longest path takes \\(O(n)\\) so his solution is \\( O(n^3) \\). If I'm not wrong, there seemed to be some inconsistency on whether \\(n\\) refered to the dimension of the matrix or the total number of elements in the matrix). He then came up with an \\(O(n)\\) by creating a custom data structure that maps the value of a cell to its index (e.g. 1: (0,2), 2: (1,1), 3: (0,1), etc.). Since you only need to iterate through the grid once to fill in your data structure and iterate through the data structure once to find the longest chain, the new solution ran in \\(O(n)\\) time. If you see the opportunity to write a class or use some design pattern, you might as well take it to demonste your knowledge of the OOP paradigm. 
...
Following the interview, the two demonstrators answered questions and gave more tips on how to have a succesful interview.
...
* Q: I failed an interview in the past. Why didn't I get any feedback on what I did wrong? A: Legal reasons. If you're trying again, try to show noticable improvement. It looks really bad if you fail to solve a problem that you failed to solve in a previous attempt.
...
* Q: What are some resumes do's and don'ts? A: Definitely include your GPA. Unless you have industry experience, keep it to a single page. 
* Q: What is the minimum GPA needed to be competitive at Google? A: Google takes a wholistic look at each candidate. I think I heard one recruiter throw out a 3.0 though.
...
In addition to the scheduled talks, a few lucky people were selected for their own practice interviews. Sadly, I wasn't on of those ~20 people. After thanking us for attending, our hosts gave everyone a bag full of Google loot which included a really nice black notebook with the Google logo embossed on the cover, sunglasses, Google socks, and a 2200mAh external battery pack. I thought the event was very well run. All the speakers and panels were very good about keeping to the schedule and there was no dead or wasted time. The biggest issue was that other Googlers kept accidentally connecting to the projector in our room. I suspect they have some sort of campus wide conference call/meeting network setup but it was pretty funny to see the projector switch from the presentation slides to some random person every now and then though. Thanks again for hosting this event, Google!